...
int noise::nextInt(engine &eng, int range)
{ // right-exclusive
	uniform_int_distribution<int> unif(0, range - 1);
	return unif(eng);
}

int noise::nextInt(engine &eng, int left, int right)
{ // right-exclusive
...
int noise::nextDiscreteLaplace(engine &eng, double scale)
{
	...
	if (nextDouble(eng) < (1 - alpha) / (1 + alpha))
		return 0;
	else
		return nextSign(eng) * (nextGeometric(eng, 1 - alpha) + 1);
}
...
int noise::sample(engine &eng, const vector<double> &weights)
{
	...
	if (cum == 0.0)
		return nextInt(eng, weights.size()); // no positive weight -> uniform sampling
...
vector<int> noise::sample(engine &eng, const vector<double> &weights, int num)
{
	...
	{
		if (cum == 0.0)
			sampled.push_back(nextInt(eng, weights.size())); // no positive weight -> uniform sampling
		else
			sampled.push_back(tools::position(nextDouble(eng, cum), cdf));
	}
	return sampled;
}