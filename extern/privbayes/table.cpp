...
int table::getDepth(int col)
{ // depth setting
	return translators[col]->depth;
	// return 1;
}
int table::getSize(int col)
{ // gets the domain size (hopefully)
...
vector<double> table::getF(const vector<double> &counts, const vector<int> &widths)
{
	...
	for (int t = 0; t < widths.size(); t++)
	{
		...
		map<int, int> now;
		now[0] = 0; // using map is faster here.
		int ceil = (size() + 1) / 2;
...
{
	marginal &margin = margins[cols];
	if (margin.counts.find(lvls) != margin.counts.end())
		return;
...
void table::printo_libsvm(const string &filename, int col, const set<int> &positives)
{
	...
	for (const auto &tuple : data)
	{
		...
		for (int t = 0; t < dim; t++)
		{
			if (t == col)
			{
				if (positives.find(tuple[t]) != positives.end())
					output = "+1" + output;
				else
					output = "-1" + output;
			}
			else
				output += " " + translators[t]->int2libsvm(tuple[t], index);
		}
		fsvm << output << endl;
	}
	fsvm.close();
