...
def lazy_em_with_rejection(
    qualities: dict,
    eps: float,
    sensitivity: float,
    is_valid,
    permanent_mask: set | None = None,
):
    """EM with lazy post-selection rejection (Adjuvant §4(a)).

    Computes the softmax distribution once from `qualities` (the only
    data-dependent step, costing ``eps^2/2`` in zCDP), then samples from it.
    Each pick is validated by ``is_valid(cl) -> (valid: bool, permanent: bool)``;
    rejected picks are excluded from the support and the distribution is
    renormalised — pure post-processing of an already-released DP query, so no
    extra privacy budget is consumed.

    Parameters
    ----------
    qualities : dict
        Candidate → score. Should already exclude entries in `permanent_mask`
        if you want to skip computing scores for them; otherwise they are
        filtered here.
    eps : float
        Per-round EM epsilon.
    sensitivity : float
        Quality-function sensitivity.
    is_valid : callable
        ``is_valid(cl) -> (valid, permanent_reject)``. ``permanent`` should be
        True when the rejection survives across rounds (e.g.\\ the candidate's
        own domain already exceeds the global cap, so growing the model can
        only make it worse).
    permanent_mask : set, optional
        Cross-round mask. Mutated in place: candidates returning
        ``permanent=True`` are added.

    Returns
    -------
    The selected key, or None if every candidate was rejected.
    """
    if permanent_mask is None:
        permanent_mask = set()

    keys = [k for k in qualities if k not in permanent_mask]
    if not keys:
        return None
    q = np.array([qualities[k] for k in keys])
    q = q - q.max()
    ...
    while True:
        active = np.array(
            [i for i in range(len(keys)) if i not in rejected_idx],
            dtype=np.int64,
        )
        if active.size == 0:
            return None
        ...
        if s <= 0:
            return None
        ap = ap / s
        i = int(active[np.random.choice(active.size, p=ap)])
        cl = keys[i]
        valid, permanent = is_valid(cl)
        if valid:
            return cl
        rejected_idx.add(i)
        if permanent:
...
def hypothetical_jt_size_mb(
    ...
    elim_max_attempts: int = 10,
) -> float:
    """Total JT max-clique size in MB after triangulating ``fitted ∪ {candidate}``.

    Mirrors the upstream ``hypothetical_model_size`` from Private-PGM AIM
    ([aim.py:23-25](private-pgm-ref/mechanisms/aim.py#L23-L25)) which builds
    the full GraphicalModel to measure the post-triangulation JT, instead of
    summing input clique sizes (which is a strict under-estimate).
    Uses few elimination-order attempts since this is a filter, not the final fit.
    """
    from ....graph.mirror_descent import build_junction_tree
    from ....graph.hugin import get_clique_domain

    cliques = list(fitted_cliques)
    if candidate is not None:
        cliques.append(candidate)
    moral = _build_moral_graph_from_cliques(cliques, attrs)
    _, jt_cliques, _ = build_junction_tree(
        [],
        attrs,
        tree_mode="hugin",
        compress=True,
    ...
    return sum(get_clique_domain(c, attrs) for c in jt_cliques) * 8 / 1e6
